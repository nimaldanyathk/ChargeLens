...
mode; on the free tier the first request after idle can take ~50s to
wake).
...
Either way, open http://localhost:8000 and the demo data seeds itself on
...
flowchart TB
    ...
    subgraph agent["Investigation agent (bounded, read-only)"]
      ...
      econ["Dispute economics<br/>per-case expected value<br/>win-prob = calibrated x cap x evidence"]:::ml
...
The scorer is a calibrated GBM, not an LLM, because dispute risk needs
auditable probabilities and per-feature attributions. The only LLM step
is drafting language, and it is gated: the grounding check discards any
draft that introduces a number or identifier not on the record, falling
back to the deterministic letter. A human approves every action, and the
agent's tool registry is the sole dispatch path, so money movement and
record edits are unreachable rather than merely forbidden.
...
4. **Economics.** The decision to fight is an expected-value calculation,
   not a raw score:
   `EV(contest) = p_win * (1 - pre_arb_haircut) * amount - fee - ops`.
   ...
   Razorpay ecosystem it is not refunded even when you win. A small
   dispute at high confidence can still be uneconomic to contest, and
   the break-even win probability falls as the amount rises.
5. **Evidence and draft.** Every evidence item cites the exact source
...
numbers on the current model: precision 86.5% and recall 84.0% at the
contest threshold, ROC-AUC 0.939, PR-AUC 0.898, each reported with a 95%
bootstrap confidence interval. Just as prominently: **85 honest customers
would have been flagged, at an estimated cost of Rs 1.06L**. False
positives are a cost, not a rounding error; the threshold was chosen to
minimize total rupee cost, not to maximize a leaderboard metric.

The same pipeline is also retrained on a time-ordered split and reported
side by side, so out-of-time performance is measured, not assumed.

Because the data is synthetic, these numbers measure the pipeline's
discipline, not real-world performance; the model card says so, and the
demo says so. The methodology (leak-free splits, calibration before
...
- **Grounding gate.** A perturbation benchmark corrupts 100 clean drafts
  across a fact-level taxonomy (swapped amounts, dates, IDs, fabricated
  quantities, unsupported numbers). The numeric/identifier gate catches
  **100% of in-scope corruptions with a 0% false-block rate**. It
  honestly reports what it cannot see: a purely qualitative fabrication
  introduces no new number or ID, so that family is counted separately as
  out-of-scope rather than excluded to inflate the headline, which is
  exactly why the deterministic generator is the default and a human
  reviews every letter.
- **Injection red-team.** 17 hostile `claim_description` fixtures across
  8 families (direct override, fake system tags, fence-break,
...
- **Deadlines:** every case carries `respond_by`; the queue sorts by it
  and the UI counts down. UPI disputes ride a shorter track than card
  disputes, because in India ~85% of digital payments are UPI and NPCI
  windows are days, not weeks.
- **Contest:** `GET /api/cases/{id}/contest-payload` returns a payload
  that validates against the live contest schema, using Razorpay's named
  evidence categories (`shipping_proof`, `billing_proof`,
  `access_activity_log`, and so on). With test keys configured it can be
  sent; without them it is an inspectable dry run.

To see a dispute arrive live, start the server with a webhook secret and
...
disputes, inflating the headline savings roughly 2x, and the threshold
tuning was reading the same labels it was later evaluated on. An
adversarial review caught both. The fix was a rebuilt evaluation pipeline
with a strict train/validation/test discipline and a cost matrix audited
line by line, and the honest numbers that came out were *lower*. We kept
them. That trade, a smaller number you can defend over a bigger one you
can't, is the design principle of the whole product.
...
backend/
  app/            FastAPI app: cases, dashboard, analytics, webhooks
    ...
    evidence/     evidence assembly, CE 3.0, dossier PDF
    integrations/ Razorpay webhook verification + contest payloads
    llm/          deterministic generator, grounded LLM redraft, safety
    risk/         model loading and scoring
  riskmodel/      data generation, training, evaluation, robustness
  tests/          100 tests: guardrails, API, economics, webhooks,
                  costs, grounding, injection, CE 3.0, dossier, bootstrap
frontend/         React console (Vite + TypeScript)
docs/MODEL_CARD.md   data, metrics, decision math, safety, limitations
docs/RESEARCH.md     domain research and citations behind the design
```

## Limitations

Synthetic data; win-probability priors are industry-cited anchors, not
fitted parameters; the grounding check validates numbers and identifiers,
not qualitative phrasing (which is why the deterministic generator is the
default and a human reviews everything); UPI dispute semantics are
modeled at the deadline and workflow level, not the full URCS lifecycle.
`docs/MODEL_CARD.md` records each in full.
